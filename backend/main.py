...
from models import User, Community, CommunityMember, WishlistItem, PurchaseOrder
...
with engine.connect() as conn:
    ...
    # Add related_user_id to notifications if missing
...
async def create_listing(
    ...
    if visibility == "public":
        if len(community_ids) != 1:
            raise HTTPException(status_code=400, detail="Public listing must have exactly one community")
        cid = community_ids[0]
        if cid != "neighborhood":
            comm = db.query(Community).filter(Community.id == cid).first()
            if not comm or not comm.is_public:
                raise HTTPException(status_code=400, detail="Selected community is not public")
...
async def get_listings(
    ...
    now = time.time()
    results = [l for l in listings_db if now - l.get("postedAt", 0) < LISTING_EXPIRY_SECONDS]
    ...
    def _is_visible(listing: dict) -> bool:
        ...
        if vis == "public":
            # Public listings visible to everyone, but neighborhood needs matching
            for c in lc:
                nc = _ncid(c)
                if nc == "neighborhood":
                    ...
                    if nc in all_public_ids or nc in my_community_ids:
                        return True
            return False
        else:
