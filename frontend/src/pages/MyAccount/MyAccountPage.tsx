...
export default function MyAccountPage({ onNavigate, onCommunitiesChanged, wishlistItems = [], onToggleWishlist, pendingListingId, onClearPendingListing, onAddToHistory, openListingDetail, onViewUser, categorySchemas }: MyAccountPageProps) {
  ...
  useEffect(() => {
    ...
    fetchAllOrders();
    fetchPunchlist();
  }, [fetchCommunities, fetchStats, fetchMyListings, fetchAllOrders, fetchPunchlist]);
  ...
  return (
    <section className="min-h-[calc(100vh-64px)] bg-canvas text-ink">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {accountTab === "listings" && (
          <div id="account-panel-listings" role="tabpanel" className="text-center py-16 border border-hairline rounded-md bg-surface-soft">
            <p className="text-sm text-muted">Listings tab coming next.</p>
          </div>
        )}

        {accountTab === "saved" && (
          <div id="account-panel-saved" role="tabpanel" className="text-center py-16 border border-hairline rounded-md bg-surface-soft">
            <p className="text-sm text-muted">Saved tab coming next.</p>
          </div>
        )}
